...
exports.calculateExpressDiscount = calculateExpressDiscount;
/**
 ...
 */
function calculateExpressDiscount(passengerProfile, estimatedTotal, config, budget) {
    ...
    if (!isEligibleForExpress)
        return result;
    result.allowed = true;
    // 3. Discount Application (Bonus)
    if (!config.isExpressBonusEnabled)
        return result;
