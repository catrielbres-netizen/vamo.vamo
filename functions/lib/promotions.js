...
exports.applyPromotionV1 = exports.getAvailablePromotionsV1 = void 0;
 ...
 */
async function checkPromotionEligibility(promotion, user, context) {
 ...
 */
exports.getAvailablePromotionsV1 = (0, https_1.onCall)({ cors: true, region: 'us-central1' }, async (request) => {
    if (!request.auth)
        throw new https_1.HttpsError('unauthenticated', 'Debes iniciar sesión.');
    const { context, amount, city } = request.data;
 ...
 */
exports.applyPromotionV1 = (0, https_1.onCall)({ cors: true, region: 'us-central1' }, async (request) => {
    if (!request.auth)
        throw new https_1.HttpsError('unauthenticated', 'Debes iniciar sesión.');
    const { promotionId, contextId, // e.g. transactionId for topup, or rideId
    amount, city } = request.data;
    const userId = request.auth.uid;
    const db = (0, firebaseAdmin_1.getDb)();
    ...
    try {
        const result = await db.runTransaction(async (tx) => {
            ...
            const promo = { id: promoSnap.id, ...promoSnap.data() };
        ...
        throw new https_1.HttpsError('internal', error.message || 'Error al aplicar la promoción.');
    }
});
