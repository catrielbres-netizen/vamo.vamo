 ...
 */
export function resolvePricingMunicipality(input: {
  cityKey?: string | null;
  city?: string | null;
  lat: number;
  lng: number;
}): {
  pricingMunicipalityKey: string | null;
  method: "provided_key" | "city_alias" | "bounds_fallback" | "unresolved";
} {
  ...
  if (input.city) {
    ...
    for (const [key, def] of Object.entries(CITY_DEFINITIONS)) {
      if (def.nameMap.includes(normalized as string)) {
