...
const navLinks = [
    { href: '/municipal/dashboard', label: 'Dashboard' },
    // Future links can be added here
    // { href: '/municipal/drivers', label: 'Conductores' },
    // { href: '/municipal/reports', label: 'Reportes' },
]
...
export function MunicipalNavbar() {
  ...
  return (
    <nav className="flex items-center gap-6 border-b bg-background p-4 sticky top-0 z-10">
        ...
        <span className="hidden md:inline">Portal Municipal</span>
        {navLinks.map(link => (
            <Link 
                key={link.href}
                href={link.href}
